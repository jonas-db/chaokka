...
class ActorE(testRef: ActorRef) extends PersistentActor with AtLeastOnceDelivery {
    ...
    def updateState(evt: Event): Any = evt match {
        case UpdateEvent(from, deliveryId) => {
            if(!state.messages.contains((from,deliveryId)) || faultyALOD) { // faulty == didn't implement idempotence of ALOD, TODO: persistence of i
                ...
                if(from.contains("ActorA")) {
                    send(from, deliveryId, List("ActorD")) ///
                } else {
                    send(from, deliveryId, List())
                }
            }
            ...
            if(from.contains("ActorA")) {
                context.become(receiveActorOther)
            }
    ...
    def send(from: String, id: Long, children:List[String]) = {
        val t = (from, id)
    ...
    def receiveActorA: Receive = {
        case State => {
            //println(state)
            sender() ! (persistenceId, state.s)
        }
        case Update(from, id) => { // if from.contains("ActorA") => {
    ...
    override def receiveCommand: Receive = receiveActorA

    def receiveActorOther: Receive = {
        case State => {
            //println(state)
            sender() ! (persistenceId, state.s)
        }
        case Update(from, id) => {
            //println(s"receiveActorOther: ${context.self.path.name} from $from received update  (state=$state)")
            persist(UpdateEvent(from, id))(updateState) // We only persist that we got the message
        }
        case Confirm(from, deliveryId) => persist(ConfirmEvent(from, deliveryId))(updateState)
        //case "shutdown" => context.stop(self)
